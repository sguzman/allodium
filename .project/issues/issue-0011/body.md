...
## Projects single-select option evolution progress

Preservation-aware managed single-select evolution landed as `02d16f1f2b8b6f339c04e4b7db15621cf38d59c3`. The planner now distinguishes option-schema work from generic deferred runtime state. For explicitly `managed` Project targets, a newly added canonical option or drift in the managed name/Allodium marker of an already mapped canonical option plans exactly one `update_project_field_options` mutation and then stops, preserving the one-mutable-operation-per-board observation boundary. Existing Project targets remain review-only and never gain automatic taxonomy rewrite authority.

The mutation is deliberately replacement-safe. GitHub's field update replaces the submitted single-select option schema, so Allodium starts from the complete observed provider option sequence rather than from the canonical option list. Every observed provider option is emitted in the same order with its existing provider option ID. Mapped, still-canonical options may receive their canonical display name and deterministic `allodium:<field-id>:<option-id>` marker while retaining their existing provider color and ID. Foreign provider options and mapped provider options whose former canonical counterpart is now absent are emitted unchanged. Canonical absence therefore still has no provider-deletion meaning.

New canonical options are appended without a provider ID and carry their deterministic Allodium marker. After GitHub returns the updated field, every pre-existing provider option ID must still exist before Allodium accepts the mutation. Existing canonical mappings are verified by stable provider ID. A new mapping is created only when exactly one returned option carries that canonical Allodium marker. Missing mapped options, marker collisions, identity disappearance, or ambiguous markers are refusal/review boundaries; option names are never used to recover identity.

Observation now preserves provider option order instead of sorting options by ID. That matters because a whole-schema replacement can otherwise rewrite provider presentation accidentally even when all identities survive. The gate includes a direct observation-order test plus separate pure-merge and fake-provider mutation tests proving preservation of foreign options, canonically removed options, existing IDs, colors, provider ordering, canonical rename by stable ID, new-option marker mapping, and use of the separate Projects credential.
