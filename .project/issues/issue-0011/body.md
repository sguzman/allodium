...
The next Projects increment is the first non-destructive mutation runtime. It must keep the separate Projects authority boundary, re-check stable provider identity/state before writes, create or bind Projects only from explicit target policy, separate item membership from field-value mutation with re-observation between them, and continue refusing destructive Project/item deletion. Live provider creation remains impossible to dogfood until the separately authorized Projects credential exists.
