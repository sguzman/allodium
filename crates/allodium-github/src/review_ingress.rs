    ...
    for item in threads {
        let snapshot = SnapshotInput {
            ...
            context: ReviewSocialContext {
                ...
                anchor_comment_id: item
                    .comments
                    .nodes
                    .first()
                    .and_then(|node| node.database_id),
                ..ReviewSocialContext::default()
            },
        };
...
"#;
    let number = i64::try_from(pull_request_number).map_err(|_| {
        format!("pull request number {pull_request_number} exceeds GraphQL Int range")
    })?;
    ...
    loop {
        ...
        if !envelope.errors.is_empty() {
            let messages = envelope
                ...
                .join("; ");
            return Err(format!(
                "GitHub GraphQL review-thread query failed: {messages}"
            ));
        }
        ...
        if cursor.is_none() {
            return Err(
                "GitHub GraphQL review-thread pagination claimed a next page without an end cursor"
                    .into(),
            );
        }
    }
...
fn archive_snapshot(
    ...
    let event = ReviewSocialEvent {
        ...
        kind: format!(
            "review.{}.observed_snapshot",
            input.kind_key.replace('-', "_")
        ),
...
fn load_observed_social(path: &Path) -> Result<Option<ObservedReviewSocial>, String> {
    ...
    if observed.schema != OBSERVED_REVIEW_SOCIAL_SCHEMA_V0 {
        return Err(format!(
            "{}: unsupported review social observation schema",
            path.display()
        ));
    }
    Ok(Some(observed))
