...
pub fn plan_reviews(root: impl AsRef<Path>, remote_name: &str) -> Result<GitHubPlan, String> {
    let root = root.as_ref();
    let remote = load_remote(root, remote_name)?;
    if remote.kind != "github" {
        return Err(format!(
...
pub fn render_review_body(review: &CanonicalReview) -> String {
    format!(
...
fn plan_review(
    ...
    if !observed_path.exists() || !observed_body_path.exists() {
        operations.push(GitHubOperation {
            ...
            reason:
                "review mapping exists but the local observed pull-request snapshot is incomplete"
                    .into(),
        });
        return Ok(());
    }
    ...
    if observed.state == "merged" && review.record.state != "merged" {
        return Err(format!(
            ...
            review.record.state, mapping.number
        ));
    }
    ...
    if observed.head_ref != review.record.head {
        return Err(format!(
            ...
            observed.head_ref, review.record.head
        ));
    }
...
mod tests {
    ...
    fn changed_review_managed_fields_plan_update() {
        ...
        let text = fs::read_to_string(&review_path)
            .unwrap()
            .replace("title = \"Test review\"", "title = \"Changed review\"");
        fs::write(review_path, text).unwrap();

        let plan = plan_reviews(&root, "github").unwrap();
        assert_eq!(plan.operations.len(), 1);
        assert_eq!(plan.operations[0].action, "update_pull_request");
        assert_eq!(plan.operations[0].fields, vec!["title", "base", "state"]);
...
"#;
    ...
    fn write_review(root: &Path, state: &str, base: &str, head: &str) {
