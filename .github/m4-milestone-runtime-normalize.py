...
text, observe_count = re.subn(
    r'#\[derive\(Debug, Clone, Default, PartialEq, Eq\)\]\npub struct ObserveReport \{.*?\n\}',
    observe,
    text,
    count=1,
    flags=re.S,
)
text, apply_count = re.subn(
    r'#\[derive\(Debug, Clone, Default, PartialEq, Eq\)\]\npub struct ApplyReport \{.*?\n\}',
    apply,
    text,
    count=1,
...
if observe_count != 1 or apply_count != 1:
    raise SystemExit(f'report normalization failed: observe={observe_count}, apply={apply_count}')
