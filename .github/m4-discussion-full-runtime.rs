...
use allodium_core::github_discussion::{
    ...
    OBSERVED_DISCUSSION_CAPABILITIES_SCHEMA_V0, OBSERVED_DISCUSSION_SCHEMA_V0,
...
pub(super) fn apply_create_discussion(
    ...
    mappings.discussions.insert(canonical_id.into(), mapping.clone());
    save_discussion_mappings(root, remote_name, &mappings)?;
    write_observed_discussion_snapshot(root, remote_name, &created_snapshot)?;

    if mapping.category_node_id != category.node_id || mapping.category_slug != binding.category_slug {
        return Err(format!(
